# tree-sitter-stack-graphs definition for scala

This project defines tree-sitter-stack-graphs rules for scala using the [tree-sitter-scala][] grammar.

[tree-sitter-scala]: https://crates.io/crates/tree-sitter-scala

- [API documentation](https://docs.rs/tree-sitter-stack-graphs-scala/)
- [Release notes](https://github.com/github/stack-graphs/blob/main/languages/tree-sitter-stack-graphs-scala/CHANGELOG.md)
...
```toml
[dependencies]
tree-sitter-stack-graphs-scala = "0.1.0"
```

Check out our [documentation](https://docs.rs/tree-sitter-stack-graphs-scala/*/) for more details on how to use this library.

## Using the Command-line Program

The command-line program for `tree-sitter-stack-graphs-scala` lets you do stack graph based analysis and lookup from the command line.
...
1. _(Installed)_ Install the CLI using Cargo as follows:
   ...
   ```sh
   cargo install --features cli tree-sitter-stack-graphs-scala
   ```

   After this, the CLI should be available as `tree-sitter-stack-graphs-scala`.

2. _(From source)_ Instead of installing the CLI, it can also be run directly from the crate directory, as a replacement for a `tree-sitter-stack-graphs-scala` invocation, as follows:
...
1. Index a source folder as follows:
   ...
   ```sh
   tree-sitter-stack-graphs-scala index SOURCE_DIR
   ```
   ...
   ```sh
   tree-sitter-stack-graphs-scala status SOURCE_DIR
   ```

   To clean the database and start with a clean slate, run:

   ```sh
   tree-sitter-stack-graphs-scala clean
   ```
...
2. Run a query to find the definition(s) for a reference on a given line and column, run:
   ...
   ```sh
   tree-sitter-stack-graphs-scala query definition SOURCE_PATH:LINE:COLUMN
   ```
...
- Builtins sources and configuration are defined in `src/builtins.scala` and `builtins.cfg` respectively.
